# 📘 UNIVERSIDAD AUTÓNOMA DE BAJA CALIFORNIA
## Facultad de Ciencias Químicas e Ingeniería
### Ingeniería en Software y Tecnologías Emergentes

---

## 🧠 PRÁCTICA III
**Portafolio de Evidencias de Propiedad Intelectual**

**💻 Proyecto:**
SmartMaq – ERP Inteligente con Inteligencia Artificial

**👤 Alumnos:**
- Bernardo Morales Ramos
- Alvaro Román Vazquez Reyes
- David Fernando Hernandez Orduno
- Cesar Cordova Contreras

**📅 Fecha de realización:**
13 de marzo de 2026

**📅 Fecha de entrega:**
17 de marzo de 2026

---

## 📌 Descripción del Proyecto
SmartMaq es una plataforma SaaS (Software as a Service) diseñada para el sector maquilador en Tijuana, Baja California. Integra automatización de procesos mediante inteligencia artificial, permitiendo optimizar operaciones empresariales y mejorar la eficiencia organizacional.

## 🎯 Objetivo
Desarrollar un portafolio digital que documente el proceso de protección de propiedad intelectual de una innovación tecnológica, considerando el entorno empresarial de Tijuana.

## 👥 Equipo de Trabajo
| Nombre | Rol |
|---|---|
| Bernardo Morales Ramos | Coordinador de Proyecto |
| Alvaro Román Vazquez Reyes | Investigador Legal |
| David Fernando Hernandez Orduno | Analista Técnico |
| Cesar Cordova Contreras | Diseñador de Estrategia |
| Cesar Cordova Contreras | Documentalista Digital |

## 💡 Resumen Ejecutivo
SmartMaq es un sistema ERP basado en la nube que incorpora:
- Automatización de procesos mediante IA
- Procesamiento de datos no estructurados
- Integración con sistemas empresariales
- Optimización de inventarios y operaciones

Su enfoque está dirigido a la industria maquiladora, permitiendo reducir errores humanos y aumentar la productividad.

---

## ⚖️ Marco Normativo y Clasificación de PI
El proyecto integra una estrategia de protección híbrida:
- 📜 **Derecho de Autor** → Software (INDAUTOR)
- 🏷️ **Marca** → Identidad comercial (IMPI)
- 🔒 **Secreto Industrial** → Algoritmos de IA

---

## 🔧 Descripción Técnica
El sistema SmartMaq funciona mediante:
- Arquitectura en la nube (SaaS)
- Backend basado en microservicios
- Motor de automatización visual (low-code)
- Integración con inteligencia artificial

Permite procesar documentos, automatizar flujos y ejecutar acciones en tiempo real.

---

## 🔍 Búsqueda de Anterioridades
Se utilizaron las siguientes bases de datos:
- IMPI
- Google Patents
- USPTO
- Espacenet

**Resultados:**
- No se encontraron marcas idénticas
- Existen soluciones similares, pero no adaptadas al entorno maquilador mexicano

---

## 📊 Estrategia de Protección
Se seleccionó una estrategia binacional:

| Alcance | Descripción |
|---|---|
| México + USA | Protección en mercado maquilador y exportación |

**Análisis FODA**
- **Fortalezas:** Protección legal del software, Identidad comercial sólida
- **Oportunidades:** Crecimiento en sector industrial
- **Debilidades:** Costos legales
- **Amenazas:** Competencia internacional

---

## 📄 Simulación de Registro
Se contempla:
- Registro de software ante INDAUTOR
- Registro de marca ante IMPI

*Los documentos son simulados con fines académicos.*

---

## 🧠 Reflexión
El desarrollo tecnológico requiere no solo innovación, sino también protección legal adecuada. Este proyecto permitió comprender la importancia de la propiedad intelectual en la creación de soluciones tecnológicas competitivas.

## 📚 Conclusión
SmartMaq representa una solución innovadora que integra tecnología y estrategia legal, demostrando que la propiedad intelectual es un elemento clave en el desarrollo empresarial moderno.

---

## 📎 Nota
Este repositorio es de carácter académico y fue desarrollado como parte de la materia Gestión de la Innovación en la UABC.
